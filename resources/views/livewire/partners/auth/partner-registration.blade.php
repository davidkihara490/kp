<div>
                ...
                <div class="registration-body bg-white p-3" style="flex: 1; overflow: {{ $step == 3 ? 'auto' : 'visible' }};">
                    ...
                    <div class="step-content">
                        ...
                        <div class="row g-2">
                            ...
                            <!-- Add this after the points_count field -->
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label class="form-label fw-semibold mb-1" style="font-size: 0.8rem;">Number of Assistants</label>
                                    <input type="number" class="form-control form-control-sm"
                                        wire:model="assistant_count" min="1">
                                    @error('assistant_count')
                                    <div class="invalid-feedback d-block" style="font-size: 0.75rem;">{{ $message }}</div>
                                    @enderror
                                </div>
                            </div>
    ...
    <script>
        ...
        Livewire.on('i_am_responsible_updated', () => {
            const responsibleFields = document.querySelectorAll('.responsible-officer-fields input');
            if (responsibleFields.length > 0) {
                // The visibility is handled by Livewire's wire:model
            }
        });

        // Auto-update driver count when vehicle count changes and i_am_driver is checked
        document.addEventListener('livewire:load', function() {
            Livewire.hook('message.processed', (message, component) => {
                // Check if we're on step 5 and transport partner
                if (component.serverMemo.data.step === 5 && component.serverMemo.data.partnerType === 'transport') {
                    const iAmDriver = component.serverMemo.data.i_am_driver;
                    const vehicleCount = component.serverMemo.data.vehicle_count;

                    if (iAmDriver && vehicleCount > 0) {
                        // Ensure driver count is 1
                        if (component.serverMemo.data.driver_count !== 1) {
                            component.call('updateDriverCountBasedOnVehicle');
                        }
                    }
                }
            });
