        ...
        <div class="account-status-card card border-0 shadow-lg rounded-4 overflow-hidden" style="width: 750px;">
            ...
            <div class="card-body p-4">
                ...
                <div class="verification-items">
                    ...
                    <!-- Row 2: Officer Email & Phone -->
                    <div class="row g-3 mb-3">
                        <div class="col-md-6">
                            <!-- Officer Email Verification -->
                            <div class="verification-item h-100 {{ $officerEmailVerified ? 'completed' : 'pending' }}">
                                <div class="d-flex align-items-center p-3 rounded-3"
                                    style="background: {{ $officerEmailVerified ? 'rgba(40, 167, 69, 0.05)' : 'rgba(255, 193, 7, 0.05)' }};">
                                    <div class="verification-icon me-3">
                                    ...
                                    <div class="flex-grow-1">
                                        <div class="d-flex justify-content-between align-items-center">
                                            <div>
                                                <h6 class="mb-0 fw-semibold" style="font-size: 0.9rem;">Officer Email</h6>
                                                <p class="mb-0 text-muted" style="font-size: 0.8rem;">
                                                    {{ $responsibleOfficerEmail ?: 'Not provided' }}
                                                </p>
                                            </div>
                                            @if($officerEmailVerified)
                                            <span class="badge bg-success bg-opacity-10 text-success border border-success border-opacity-25 py-1 px-2"
                                            ...
                                            @elseif($responsibleOfficerEmail)
                                            <button wire:click="resendOfficerEmailVerification"
                                                class="btn btn-sm btn-outline-success py-1 px-2"
                                                style="font-size: 0.75rem;"
                                                wire:loading.attr="disabled">
                                                <span wire:loading.remove wire:target="resendOfficerEmailVerification">
                                                    Verify
                                                </span>
                                                <span wire:loading wire:target="resendOfficerEmailVerification">
                                                    <span class="spinner-border spinner-border-sm" style="width: 0.8rem; height: 0.8rem;"></span>
                                                </span>
                                            </button>
                                            @else
                                            <button wire:click="addOfficerEmail"
                                ...
                                @if(!$responsibleOfficerEmail && !$officerEmailVerified)
                                <div class="verification-info p-2 rounded-2 mt-1"
                                    style="background: rgba(108, 117, 125, 0.05); font-size: 0.75rem;">
                                    <div class="d-flex align-items-start">
                                        <i class="bi bi-info-circle text-secondary mt-1 me-2" style="font-size: 0.9rem;"></i>
                                        <div>
                                            <p class="mb-0">Add email for official communications</p>
                                        </div>
                                    </div>
                                </div>
                                @elseif(!$officerEmailVerified && $responsibleOfficerEmail)
                                <div class="verification-info p-2 rounded-2 mt-1"
                                    style="background: rgba(255, 193, 7, 0.05); font-size: 0.75rem;">
                                    <div class="d-flex align-items-start">
                                        <i class="bi bi-info-circle text-warning mt-1 me-2" style="font-size: 0.9rem;"></i>
                                        <div>
                                            <p class="mb-0">Verification link sent to officer's email</p>
                                        </div>
                                    </div>
                                </div>
                                @endif
                        ...
                        <div class="col-md-6">
                            <!-- Officer Phone Verification -->
                            <div class="verification-item h-100 {{ $officerPhoneVerified ? 'completed' : 'pending' }}">
                                <div class="d-flex align-items-center p-3 rounded-3"
                                    style="background: {{ $officerPhoneVerified ? 'rgba(40, 167, 69, 0.05)' : 'rgba(255, 193, 7, 0.05)' }};">
                                    <div class="verification-icon me-3">
                                        @if($officerPhoneVerified)
                                        <div class="verified-icon rounded-circle bg-success d-flex align-items-center justify-content-center"
                                            style="width: 32px; height: 32px;">
                                            <i class="bi bi-check-lg text-white" style="font-size: 1rem;"></i>
                                        </div>
                                    ...
                                    <div class="flex-grow-1">
                                        <div class="d-flex justify-content-between align-items-center">
                                            <div>
                                                <h6 class="mb-0 fw-semibold" style="font-size: 0.9rem;">Officer Phone</h6>
                                                <p class="mb-0 text-muted" style="font-size: 0.8rem;">{{ $responsibleOfficerPhone }}</p>
                                            </div>
                                            @if($officerPhoneVerified)
                                            <span class="badge bg-success bg-opacity-10 text-success border border-success border-opacity-25 py-1 px-2"
                                                style="font-size: 0.75rem;">
                                            ...
                                            @else
                                            <button wire:click="sendOfficerPhoneVerification"
                                                class="btn btn-sm btn-outline-success py-1 px-2"
                                                style="font-size: 0.75rem;"
                                                wire:loading.attr="disabled">
                                                <span wire:loading.remove wire:target="sendOfficerPhoneVerification">
                                                    Verify
                                                </span>
                                                <span wire:loading wire:target="sendOfficerPhoneVerification">
                                                    <span class="spinner-border spinner-border-sm" style="width: 0.8rem; height: 0.8rem;"></span>
                                                </span>
                                            </button>
                                            @endif
                                ...
                                <div class="verification-info p-2 rounded-2 mt-1"
                                    ...
                                    <div class="d-flex align-items-start">
                                        <i class="bi bi-info-circle text-warning mt-1 me-2" style="font-size: 0.9rem;"></i>
                                        <div>
                                            <p class="mb-0">Required for emergency communications</p>
                                            <p class="mb-0">SMS verification code will be sent</p>
                                        </div>
                                    </div>
                                </div>
                                @endif
