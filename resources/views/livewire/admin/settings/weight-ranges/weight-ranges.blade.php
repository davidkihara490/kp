<div>
    <div class="card">
        <div class="card-header">
            ...
            <a href="{{ route('admin.weight-ranges.create') }}" class="btn btn-success btn-sm float-right">New </a>
        </div>
        ...
        <div class="card-body">
            ...
            <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                    @forelse ($weightRanges as $weightRange)
                    <tr>
                        <td>{{ $loop->iteration }}</td>
                        <td>{{ $weightRange->label }}</td>
                        <td>{{ $weightRange->min_weight }}</td>
                        <td>{{ $weightRange->max_weight }}</td>
                        <td>{{ $weightRange->unit }}</td>
                        <td>
                            <a href="{{ route('admin.weight-ranges.edit', $weightRange->id) }}" class="btn btn-sm btn-info">Edit</a>
                            <button class="btn btn-sm btn-danger"
                                wire:click="confirm({{ $weightRange->id }})">Delete</button>
