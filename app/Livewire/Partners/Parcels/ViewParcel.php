...
class ViewParcel extends Component
{
    ...
    public $isTransportPartner = false;
    public $isOriginPudo = false;
    public $isDestinationPudo = false;
    public $isPha = false;
    ...
    protected function determineUserRoles()
    {
        ...
        // Check if user is origin PUDO (sender partner)
        $this->isOriginPudo = false;
        if ($this->isPha || $this->isPickupDropoff) {
            $userPartnerId = $user->parcelHandlingAssistant?->partner?->id ?? $user->partner?->id;
            $this->isOriginPudo = $userPartnerId === $parcel->sender_partner_id;
        }
    ...
    {
        // Only destination PUDO can verify pickup
        if (!$this->isDestinationPudo) {
            $this->showErrorModal = true;
            $this->errorMessage = "You are not authorized to verify parcel pickup. Only the destination PUDO can perform this action.";
            return;
        }
    ...
    {
        // Only origin PUDO can verify driver code
        if (!$this->isOriginPudo) {
            $this->showErrorModal = true;
            $this->errorMessage = "You are not authorized to verify driver codes. Only the origin PUDO can perform this action.";
            return;
        }
    ...
    {
        // Only destination PUDO can open pickup modal
        if (!$this->isDestinationPudo) {
            $this->showErrorModal = true;
            $this->errorMessage = "You are not authorized to verify parcel pickup. Only the destination PUDO can perform this action.";
            return;
    ...
    public function verifyDriverCode()
    {
        ...
        if ($this->latestStatus->otp == $this->driver_code) {
            ...
            session()->flash('success', 'Driver verified successfully. Parcel is now in transit.');
        } else {
    ...
    {
        // Only destination PUDO can receive parcel from driver
        if (!$this->isDestinationPudo) {
            $this->showErrorModal = true;
            $this->errorMessage = "You are not authorized to receive parcels from drivers. Only the destination PUDO can perform this action.";
            return;
        }
        ...
        try {
            ...
            Log::error('Failed to send SMS to recipient: ', [
                'error' => $th->getMessage(),
                'stack' => $th->getTraceAsString(),
            ]);
        }

        session()->flash('success', 'Parcel received from driver successfully.');
    }
    ...
    {
        // Driver and transport partners should not see payment modal
        if ($this->isDriver || $this->isTransportPartner) {
            $this->showErrorModal = true;
            $this->errorMessage = "You do not have permission to view payment information.";
            return;
        }
    ...
    {
        // Only origin PUDO can accept parcel
        if (!$this->isOriginPudo) {
            $this->showErrorModal = true;
            $this->errorMessage = "You are not authorized to accept this parcel. Only the origin PUDO can accept this parcel.";
            return;
        }
    ...
    public function shouldShowSenderInfo()
    {
        return !($this->isDriver || $this->isTransportPartner);
    }

    public function shouldShowReceiverInfo()
    {
        return !($this->isDriver || $this->isTransportPartner);
    }

    public function shouldShowPriceInfo()
    {
        return !($this->isDriver || $this->isTransportPartner);
    }

    public function shouldShowDriverVerification()
    {
        return $this->isOriginPudo && $this->parcel->current_status === Parcel::STATUS_BOOKED;
    }
    ...
    {
        return !$this->isDriver && $this->parcel->currentDriver() !== null;
    }

    public function shouldShowDriverCode()
    {
        return $this->isDriver || $this->isTransportPartner || $this->isOriginPudo;
    }

    public function shouldShowTrackingTab()
    {
        return true; // All users can see tracking
    }

    public function shouldShowPaymentsTab()
    {
        return !($this->isDriver || $this->isTransportPartner);
    }
}
